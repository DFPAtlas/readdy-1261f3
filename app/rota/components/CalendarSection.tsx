...
import { getMonthData, getShiftsForDay, isDateInDragRange } from '../calendar';
...
interface Props {
  ...
  handleLeaveClick: (e: React.MouseEvent, staffInitials: string, date: Date) => void;
  isDragging: boolean;
    ...
    <div className="bg-white rounded-xl shadow-lg p-6 border border-slate-200 mb-6">
      ...
      <div className="mb-4 p-3 bg-blue-50 rounded-lg border border-blue-200">
        <div className="flex items-center gap-2 text-blue-700 text-sm">
          ...
          <span><strong>Tip:</strong> Click and drag across multiple dates to select a date range for leave booking. Click on any leave entry (red highlighted) to edit or cancel it.</span>
        </div>
      </div>
      ...
      >
        ...
        {days.map((day) => {
          const shifts = getShiftsForDay(day, month, year, staffData);
              ...
              <div className="space-y-0.5 text-xs">
                {shifts.map((shift, idx) => (
                  <div
                    ...
                    onClick={(e) => {
                      if (shift.onLeave) {
                        handleLeaveClick(e, shift.staff, new Date(year, month, day));
                      }
                    }}
                    ...
                    title={shift.onLeave ? `${shift.staff} - ${shift.leaveType} (Click to edit)` : `${shift.staff} - ${shift.type === 'D' ? 'Day' : 'Night'} Shift`}
                  >
      ...
      <div className="mt-6 pt-6 border-t border-slate-200">
        ...
        {showLegend && (
          <div className="space-y-4">
            <div>
              <p className="text-xs font-medium text-slate-500 mb-2 uppercase tracking-wide">Staff Shifts</p>
            ...
            <div>
              <p className="text-xs font-medium text-slate-500 mb-2 uppercase tracking-wide">Calendar Indicators</p>
              <div className="flex flex-wrap gap-4">
                <div className="flex items-center gap-2">
                  <div className="w-6 h-6 bg-blue-100 border-2 border-blue-400 rounded-lg"></div>
                  <span className="text-sm text-slate-700">Selected/Drag Range</span>
                </div>
                <div className="flex items-center gap-2">
                  <div className="w-6 h-6 bg-slate-50 border-2 border-slate-200 rounded-lg"></div>
                  <span className="text-sm text-slate-700">Weekend</span>
                </div>
                <div className="flex items-center gap-2">
                  <div className="w-6 h-6 bg-white border-2 border-slate-200 rounded-lg"></div>
                  <span className="text-sm text-slate-700">Weekday</span>
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="mt-6 flex justify-center">
        <button className="flex items-center gap-2 px-6 py-3 bg-slate-700 text-white rounded-lg hover:bg-slate-800 transition-colors font-medium shadow-md hover:shadow-lg whitespace-nowrap cursor-pointer">
          <div className="w-5 h-5 flex items-center justify-center">
            <i className="ri-printer-line text-lg"></i>
          </div>
          <span>Print Rota</span>
        </button>
      </div>
    </div>
  );
